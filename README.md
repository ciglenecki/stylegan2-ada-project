...
https://code.visualstudio.com/docs/devcontainers/attach-container#_attached-container-configuration-reference

Alias to get an ID of the current running stylegan2 docker container:
```
docker_curr # same as: docker ps -a | grep $(whoami)-stylegan2 | cut -d " " -f1
```
...
```
docker run --gpus device=0 -ti --name=$(whoami)-stylegan2 -v ~/projects:/root/projects mciglenecki-stylegan2-ada:latest
```

Connect to a docker container

```
docker container attach $(docker_curr) # or: docker_curr_connect
```
