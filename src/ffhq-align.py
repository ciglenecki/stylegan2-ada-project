...
class CFA(nn.Module):
    def __init__(self, output_channel_num, checkpoint_name=None):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, dilation=1, padding=1),
            ...
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(128, 256, kernel_size=3, dilation=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=3, dilation=1, padding=1),
            nn.ReLU(inplace=True),
        ...
        self.CFM_features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(256, self.stage_channel_num, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
        )
        ...
        for _ in range(1, self.stage_num):
            stages.append(
                self.make_stage(self.stage_channel_num + self.output_channel_num)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=256,
        type=int,
        help="size of aligned output (default: 256)",
    )
    parser.add_argument(
        "-t",
        "--transform_size",
